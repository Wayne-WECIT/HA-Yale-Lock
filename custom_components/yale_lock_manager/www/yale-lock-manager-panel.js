...
class YaleLockManagerPanel extends HTMLElement {
  ...
  _updateSlotFromEntityState(slot) {
    ...
    if (this._expandedSlot === slot) {
      ...
      const chipsContainer = this.querySelector(`#notification-services-${slot}`);
      if (chipsContainer) {
        const tempUser = { ...user, notification_services: notificationServices };
        chipsContainer.innerHTML = this.renderNotificationServiceChips(slot, tempUser);
      }
  ...
  render() {
    ...
    const currentTimeIso = stateObj.attributes.current_time_iso;
      ...
      <div class="panel-container">
        <div class="header">
          ...
          <div class="controls">
            ...
            <button class="secondary" onclick="panel.showExportConfirm()">Export</button>
          </div>
        </div>
        ...
        <div style="text-align: center; margin-bottom: 12px;">
          ${this._showExportConfirm ? `
            <div style="background: var(--warning-color-background, rgba(255, 152, 0, 0.1)); border: 1px solid var(--warning-color, #ff9800); border-radius: 4px; padding: 12px; margin-bottom: 12px;">
              ...
              <div style="display: flex; gap: 8px; justify-content: center;">
                <button onclick="panel.confirmExportDownload()">Download</button>
                <button class="secondary" onclick="panel.cancelExportConfirm()">Cancel</button>
              </div>
            </div>
          ` : ''}
          ${this._showImportConfirm ? `
            <div style="background: var(--warning-color-background, rgba(255, 152, 0, 0.1)); border: 1px solid var(--warning-color, #ff9800); border-radius: 4px; padding: 12px; margin-bottom: 12px;">
              ...
              <div style="display: flex; gap: 8px; justify-content: center;">
                <button onclick="panel.confirmImportChooseFile()">Choose backup file</button>
                <button class="secondary" onclick="panel.cancelImportConfirm()">Cancel</button>
              </div>
            </div>
          ` : ''}
          <button class="secondary" onclick="panel.showExportConfirm()">Export backup</button>
          <button class="secondary" onclick="panel.showImportConfirm()">Import backup</button>
        </div>
    ...
    return allServices.map(service => {
      ...
      return `
        <span 
          ...
          onclick="panel.toggleNotificationService(${slot}, '${service.id}').catch(err => console.error('Error toggling service:', err))"
          title="${service.id}"
        >
  ...
  async toggleNotification(slot, checked) {
    try {
      ...
      const chipsContainer = this.querySelector(`#notification-services-${slot}`);
      const selectedChips = chipsContainer ? chipsContainer.querySelectorAll('.notification-chip.selected') : [];
      const selectedServices = Array.from(selectedChips).map(chip => {
        const onclick = chip.getAttribute('onclick');
        const match = onclick.match(/'([^']+)'/);
        return match ? match[1] : null;
      }).filter(Boolean);
      const servicesArray = selectedServices.length > 0 ? selectedServices : ['notify.persistent_notification'];
  ...
  async toggleNotificationService(slot, serviceId) {
    try {
      // Get current selected services from chips container
      const chipsContainer = this.querySelector(`#notification-services-${slot}`);
      if (!chipsContainer) return;
      
      // Get selected services by checking which chips are selected
      const selectedChips = chipsContainer.querySelectorAll('.notification-chip.selected');
      const currentServices = Array.from(selectedChips).map(chip => {
        const onclick = chip.getAttribute('onclick');
        const match = onclick.match(/'([^']+)'/);
        return match ? match[1] : null;
      }).filter(Boolean);
      ...
      await this._hass.callService('yale_lock_manager', 'set_notification_enabled', {
        ...
        notification_services: newServices.length > 0 ? newServices : ['notify.persistent_notification']
      });
      
      // Update chips display
      const user = this.getUserData().find(u => u.slot === slot);
      if (user) {
        // Temporarily update user data for rendering
        const tempUser = { ...user, notification_services: newServices };
        chipsContainer.innerHTML = this.renderNotificationServiceChips(slot, tempUser);
      }
  ...
  triggerImportBackup() {
    const input = this.querySelector('#import-file-input');
    if (input) {
      input.value = '';
